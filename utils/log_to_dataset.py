...
path_file = os.path.join(path_dataset, 'LF_small_loop_DIST.log')
out_dataset = os.path.join(path_to_proj, 'dataset_LanePose_sim', 'test_default_r4')
...
for i, entry in enumerate(data[0]):
    pic_name = '_'.join(['default', str(i), str(i), 'sim.jpg'])
    pic_path = os.path.join(out_images, pic_name)
    img = Image.fromarray(entry.astype('uint8'))
